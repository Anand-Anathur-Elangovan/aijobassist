...
export async function POST(req: NextRequest) {
  try {
    ...
    const bodyCutoff = body.cutoff_marks ?? {};
    const input: StudentInput = {
      ...
      cutoff_marks: {
        math:      bodyCutoff.math      ? Number(bodyCutoff.math)      : undefined,
        physics:   bodyCutoff.physics   ? Number(bodyCutoff.physics)   : undefined,
        chemistry: bodyCutoff.chemistry ? Number(bodyCutoff.chemistry) : undefined,
        neet:      bodyCutoff.neet      ? Number(bodyCutoff.neet)      : undefined,
      },
      ...
      favorite_colleges: Array.isArray(body.favorite_colleges) ? body.favorite_colleges : [],
    };
    ...
    try {
      ...
      const { data: profile } = await sb
        ...
        .upsert({
          ...
          favorite_colleges: input.favorite_colleges ?? [],
      ...
      await sb.from("career_predictions").insert({
        ...
        favorite_college_analysis: result.favorite_college_analysis ?? null,
